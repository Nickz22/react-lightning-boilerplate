...
export default function App() {
  ...
  let actionInput
  let nonActionInput
  const [view, setView] = useState([getUpdatedView(step)])
  /**
  ...
  function getUpdatedView(index) {
    const viewMap = {
      0: <div></div>,
      1: [
        <ProgressBar boxes={sequenceSteps} />,
        <div id="modal-container">
          <SequenceDetail viewMap={modalContent} />
        </div>,
      ],
      2: [
        <ProgressBar boxes={sequenceSteps} />,
        <div id="modal-container" className="show">
          <SequenceEntryCriteria viewMap={modalContent} />
        </div>,
      ],
      3: [
        <ProgressBar boxes={sequenceSteps} />,
        <div id="modal-container">
          <SequenceExitCriteria viewMap={modalContent} />
        </div>,
      ],
      4: [
        <ProgressBar boxes={sequenceSteps} />,
        <div id="modal-container" className="show">
          <SequenceAction viewMap={actionModalContent} />
        </div>,
      ],
    }
  ...
  function addStep(event) {
    const id = parseInt(event['info']['id'])
    const actionInsert = sequenceSteps.pop()
    sequenceSteps.push(<div className="line-connector" id={id - 0.5}></div>)
    sequenceSteps.push(
      <Box label={event['info']['name']} type={event['info']['type']} onclick={showModal} id={id} />,
    )
  ...
  function stepExists(event) {
    ...
    const boxMap = getStepMap(sequenceSteps)
    if (boxMap.has(id)) {
      ...
      boxMap.set(
        ...
        <Box
          id={id}
          label={event['info']['name']}
          type={event['info']['type']}
          onclick={showModal}
        />,
      )
      const orderedMap = new Map([...boxMap.entries()].sort())
  ...
  function saveAction(label) {
    const action = {}
    action['CadenceAction_ID__c'] = document.getElementById(
      'action-results',
